...
Public Class FormReturRI
    ...
    Sub NoRetur()
        Try
            konek()
            CMD = New SqlCommand("select max(notaretur) as notaretur from ap_returinap1 where tanggal='" & Format(DTPTanggalTrans.Value, "yyyy/MM/dd") & "' and kdbagian='" & My.Settings.pkdapo & "'", CONN)
            DR = CMD.ExecuteReader
            DR.Read()
            If IsDBNull(DR.Item("notaretur")) Then
                txtNoRetur.Text = My.Settings.pkdapo + "-" + "RT" + Format(DTPTanggalTrans.Value, "ddMMyy") + "001"
            Else
                txtNoRetur.Text = Microsoft.VisualBasic.Right(DR.Item("notaretur").ToString, 3) + 1
                If Len(txtNoRetur.Text) = 1 Then
                    txtNoRetur.Text = My.Settings.pkdapo + "-" + "RT" + Format(DTPTanggalTrans.Value, "ddMMyy") + "00" & txtNoRetur.Text & ""
                ElseIf Len(txtNoRetur.Text) = 2 Then
                    txtNoRetur.Text = My.Settings.pkdapo + "-" + "RT" + Format(DTPTanggalTrans.Value, "ddMMyy") + "0" & txtNoRetur.Text & ""
                ElseIf Len(txtNoRetur.Text) = 3 Then
                    txtNoRetur.Text = My.Settings.pkdapo + "-" + "RT" + Format(DTPTanggalTrans.Value, "ddMMyy") + "" & txtNoRetur.Text & ""
                End If
            End If
    ...
    Sub addBarang()
        ...
        DRWReturObatInap("kdbagian") = My.Settings.pkdapo
    ...
    Private Sub FormReturRI_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub FormReturRI_Resize(sender As Object, e As EventArgs) Handles Me.Resize
    ...
    Private Sub DoubleTextBox1_KeyPress(sender As Object, e As KeyPressEventArgs)
        If e.KeyChar = Chr(13) Then
            SendKeys.Send("{TAB}")
        End If
    End Sub
        ...
        If MessageBox.Show("Data tersebut sudah benar?", "Konfirmasi", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
            ...
            Try
                ...
                If My.Settings.psts_stok = "1" Then
                    For i = 0 To gridDetailObat.RowCount - 2
                        'konek()
                        sqlReturObatInap = sqlReturObatInap + vbCrLf + "UPDATE barang_farmasi SET " & memStok & "=" & memStok & "+" & Num_En_US((Num_En_US(gridDetailObat.Rows(i).Cells("jmlretpkt").Value) + Num_En_US(gridDetailObat.Rows(i).Cells("jmlretnpkt").Value))) & " WHERE kd_barang='" & Trim(gridDetailObat.Rows(i).Cells("kd_barang").Value) & "'"
                        'CMD.ExecuteNonQuery()
                    Next
                End If
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    ...
    Private Sub txtJmlHargaPaket_TextChanged(sender As Object, e As EventArgs) Handles txtJmlHargaPaket.TextChanged
    ...
    Private Sub txtIurPasien_TextChanged(sender As Object, e As EventArgs) Handles txtIurPasien.TextChanged
