    ...
    Sub TampilStok1()
        ...
        Try
            ...
            DA = New OleDb.OleDbDataAdapter("select RTRIM(LTRIM(jenis_obat.jns_obat)) as nmjenis, 
                    RTRIM(LTRIM(barang_farmasi.nama_barang)) as nmbarang,  0 as stokawal, 
                    0 as terimagdg, 0 as terima1, 0 as terima2, 0 as terima3, 0 as terima4, 0 as terima5, 
                    0 as terima6, 0 as terima7, 0 as terimaunt, 0 as retcekout, 0 as retrajal, 
                    0 as retinap, 0 as kormasuk, 0 as ttlmasuk, 0 as jualresep, 0 as jualbebas, 
                    0 as mutasi, 0 as retgudang, 0 as korkel, 0 as ttlkeluar, 0 as jmlstok, 
                    RTRIM(LTRIM(barang_farmasi.kd_satuan_kecil)) as nmsatuan, 
                    Round(barang_farmasi.harga_satuan,0) as harga, 
                    0 as jmlharga,
                    RTRIM(LTRIM(barang_farmasi.kd_barang)) as kdbarang, 
                    RTRIM(LTRIM(barang_farmasi.kd_jns_obat)) as kdjenis from barang_farmasi INNER JOIN jenis_obat ON barang_farmasi.kd_jns_obat=jenis_obat.kd_jns_obat where LEFT(kd_barang,2)='NW' order by jenis_obat.jns_obat,barang_farmasi.nama_barang", CONN)
            ...
            '''''''''''Retur Rawat JALAN
            DA = New OleDb.OleDbDataAdapter("select RTRIM(LTRIM(kd_barang)) as kdbarang, total_qty as jml from ap_retur_detail where kd_bagian='" & pkdapo & "' and Month(tanggal)='" & Month(DTPBulan.Value) & "' and Year(tanggal)='" & Year(DTPTahun.Value) & "' order by kd_barang", CONN)
            ...
            If BDStokPerbulan.Count > 0 Then
                ...
                For i = 1 To BDStokPerbulan.Count
                    DRWStokPerbulan = BDStokPerbulan.Current
                    DRWStokPerbulan("jmlstok") = Val(DRWStokPerbulan.Item("stokawal")) + Val(DRWStokPerbulan.Item("terimagdg")) + Val(DRWStokPerbulan.Item("terimaunt")) + Val(DRWStokPerbulan.Item("retcekout")) + Val(DRWStokPerbulan.Item("retrajal")) + Val(DRWStokPerbulan.Item("retinap")) + Val(DRWStokPerbulan.Item("kormasuk")) - Val(DRWStokPerbulan.Item("jualresep")) - Val(DRWStokPerbulan.Item("jualbebas")) - Val(DRWStokPerbulan.Item("mutasi")) - Val(DRWStokPerbulan.Item("korkel")) - Val(DRWStokPerbulan.Item("retgudang"))
                    DRWStokPerbulan("ttlmasuk") = Val(DRWStokPerbulan.Item("stokawal")) + Val(DRWStokPerbulan.Item("terimagdg")) + Val(DRWStokPerbulan.Item("terimaunt")) + Val(DRWStokPerbulan.Item("retcekout")) + Val(DRWStokPerbulan.Item("retrajal")) + Val(DRWStokPerbulan.Item("retinap")) + Val(DRWStokPerbulan.Item("kormasuk"))
    ...
    Private Sub Button5_Click(sender As Object, e As EventArgs) Handles btnPreview.Click
        ...
        For i = 0 To gridObat.RowCount - 2
            If Not IsDBNull(gridObat.Rows(i).Cells(0).Value) Then
                dtReport.Rows.Add(gridObat.Rows(i).Cells("nmjenis").Value, gridObat.Rows(i).Cells("nmbarang").Value, gridObat.Rows(i).Cells("stokawal").Value, gridObat.Rows(i).Cells("terimagdg").Value, gridObat.Rows(i).Cells("terimaunt").Value, gridObat.Rows(i).Cells("retcekout").Value, gridObat.Rows(i).Cells("retrajal").Value, gridObat.Rows(i).Cells("retinap").Value, gridObat.Rows(i).Cells("kormasuk").Value, gridObat.Rows(i).Cells("jualresep").Value, gridObat.Rows(i).Cells("jualbebas").Value, gridObat.Rows(i).Cells("mutasi").Value, gridObat.Rows(i).Cells("retgudang").Value, gridObat.Rows(i).Cells("korkel").Value, gridObat.Rows(i).Cells("jmlstok").Value, gridObat.Rows(i).Cells("nmsatuan").Value, gridObat.Rows(i).Cells("harga").Value, gridObat.Rows(i).Cells("jmlharga").Value)
            End If
        Next
