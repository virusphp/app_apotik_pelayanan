...
Public Class FormPenjualanNonResep
    ...
    Private Sub btnSimpan_Click(sender As Object, e As EventArgs) Handles btnSimpan.Click
        ...
        If MessageBox.Show("Data Penjualan sudah benar ...?", "", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
            ...
            Try
                ...
                '''''''''''''''''''''''''''''''''''''''''TRAN KE APOTEK'''''''''''''''''''''''''''''''''''''' 
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''Simpan ap_jualbbs1
                sqlPenjualanObatNonResep = "insert into ap_jualbbs1 (kdbagian, nmbagian, kdkasir, nmkasir, 
                    tanggal, nota, kdkons, nmkons, nama, kddokter, nmdokter, jmltotal, tuslah, jmlharga1, 
                    potongan, jmlharga2, bulat, jmlnet, posting, jam, diserahkan, kdpelanggan) 
                    values ('" & pkdapo & "', '" & pnmapo & "', '" & Trim(FormLogin.LabelKode.Text) & "', 
                    '" & Trim(FormLogin.LabelNama.Text) & "','" & Format(DTPTanggalTrans.Value, "yyyy/MM/dd") & "',
                    '" & Trim(txtNota.Text) & "', '" & Trim(kdKonsumen) & "', '" & Trim(NamaKonsumen) & "', 
                    '" & Trim(txtNamaPasien.Text) & "', '" & Trim(kdDokter) & "', '" & Trim(NamaDokter) & "', 
                    '" & Num_En_US(txtGrandTotal1.DecimalValue) & "', '" & Num_En_US(txtGrandTuslah.DecimalValue) & "', 
                    '" & Num_En_US(txtGrandTotal2.DecimalValue) & "', '" & Num_En_US(txtGrandJumlahPotongan.DecimalValue) & "', 
                    '" & Num_En_US(txtGrandTotal3.DecimalValue) & "', '" & Num_En_US(txtGrandTotalBulat.DecimalValue) & "', 
                    '" & Num_En_US(txtGrandJumlahHarga.DecimalValue) & "', '1', '" & Format(DTPJam.Value, "HH:mm:ss") & "', 
                    'B','" & Trim(txtKdPelanggan.Text) & "' )"
                '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''Simpan ap_jualbbs2
                For i = 0 To gridDetailObat.RowCount - 2
                    sqlPenjualanObatNonResep = sqlPenjualanObatNonResep & vbCrLf & "INSERT INTO ap_jualbbs2(
                        kdbagian,nmbagian,kdkasir,nmkasir,tanggal,nota,kdkons,nmkons,nama, kdDokter, 
                        nmdokter, urut, idx_barang, kd_barang, nama_barang, kd_jns_obat, jns_obat, 
                        kd_kel_obat, kd_gol_obat, Generik, harga, jml, nmsatuan, jmltotal, tuslah, 
                        jmlharga, senpot, potongan, jmlnet, posting,diserahkan,hpp,racik,jmlracik,jam, 
                        rek_p, stsetiket, kdpelanggan) 
                        VALUES ('" & pkdapo & "', '" & pnmapo & "', '" & Trim(FormLogin.LabelKode.Text) & "', 
                        '" & Trim(FormLogin.LabelNama.Text) & "',
                        '" & Format(DTPTanggalTrans.Value, "yyyy/MM/dd") & "','" & Trim(txtNota.Text) & "', 
                        '" & Trim(kdKonsumen) & "', '" & Trim(NamaKonsumen) & "', '" & Trim(txtNamaPasien.Text) & "', 
                        '" & Trim(kdDokter) & "', '" & Trim(NamaDokter) & "', '" & i + 1 & "', 
                        '" & Trim(gridDetailObat.Rows(i).Cells("idx_barang").Value) & "', '" & Trim(gridDetailObat.Rows(i).Cells("kdbarang").Value) & "', 
                        '" & Rep(Trim(gridDetailObat.Rows(i).Cells("nmbarang").Value)) & "', '" & Trim(gridDetailObat.Rows(i).Cells("kdjenis").Value) & "', 
                        '" & Trim(gridDetailObat.Rows(i).Cells("nmjenis").Value) & "', '" & Trim(gridDetailObat.Rows(i).Cells("kdkel").Value) & "', 
                        '" & Trim(gridDetailObat.Rows(i).Cells("kdgol").Value) & "', '" & Trim(gridDetailObat.Rows(i).Cells("generik").Value) & "', 
                        '" & Num_En_US(gridDetailObat.Rows(i).Cells("harga").Value) & "', '" & Num_En_US(gridDetailObat.Rows(i).Cells("jml").Value) & "', 
                        '" & Trim(gridDetailObat.Rows(i).Cells("nmsatuan").Value) & "', '" & Num_En_US(gridDetailObat.Rows(i).Cells("jmltotal").Value) & "', 
                        '" & Num_En_US(gridDetailObat.Rows(i).Cells("tuslah").Value) & "', '" & Num_En_US(gridDetailObat.Rows(i).Cells("jmlharga").Value) & "', 
                        '" & Num_En_US(gridDetailObat.Rows(i).Cells("senpot").Value) & "', '" & Num_En_US(gridDetailObat.Rows(i).Cells("potongan").Value) & "', 
                        '" & Num_En_US(gridDetailObat.Rows(i).Cells("jmlnet").Value) & "', '" & Num_En_US(gridDetailObat.Rows(i).Cells("posting").Value) & "', 
                        'B', '" & Num_En_US(gridDetailObat.Rows(i).Cells("hpp").Value) & "', '" & Trim(gridDetailObat.Rows(i).Cells("racik").Value) & "', 
                        '" & Num_En_US(gridDetailObat.Rows(i).Cells("jmlracik").Value) & "', '" & Format(DTPJam.Value, "yyyy/MM/dd HH:mm:ss") & "', 
                        '" & Trim(gridDetailObat.Rows(i).Cells("rek_p").Value) & "', '" & Trim(gridDetailObat.Rows(i).Cells("stsEtiket").Value) & "',
                        '" & Trim(txtKdPelanggan.Text) & "'
                        )"
                Next
                '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

                '''''''''''''''''''''''''''''''''''' TRANSAKSI PELANGGAN UPDATE OR INSERT
                sqlPenjualanObatNonResep = sqlPenjualanObatNonResep & vbCrLf & strSql
    ...
    Private Sub FormPenjualanNonResep_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Dispose()
        FormInfoResepObat.Dispose()
        FormPelanggan.Dispose()
    End Sub

    Private Sub cmbKonsumen_LostFocus(sender As Object, e As EventArgs) Handles cmbKonsumen.LostFocus
        'cariKonsumen()
        'If NamaKonsumen = "UMUM" Or NamaKonsumen = "KARYAWAN" Then
        '    txtNamaPasien.Enabled = True
        'Else
        '    cmbKonsumen.Focus()
        '    txtNamaPasien.Enabled = False
