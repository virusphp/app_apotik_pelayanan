...
Public Class FormEditPenjualanResep
    ...
    Dim StatusRawat, JenisRawat, KdPenjamin, kdDokter, kdPoliklinik, kdTempatTidur, Stok, Generik, KdJenisObat, kdPabrik, kdKelompokObat,
        kdGolonganObat, NamaPenjamin, NamaDokter, kdTakaran, kdWaktu, kdKeterangan, JenisObat, memStok, kdSubUnit, Posting, nmPaket, kdRekening,
        kdKeteranganModel3, kdICDSKU, kdICDRM, noKartu, noSep As String
    Public nmSubUnit, bilang, modelEtiket, noRegister As String
    ...
    Dim BDEtiketModel4 As New BindingSource
    Dim DSEtiketModel4 As New DataSet
    Dim DRWEtiketModel4 As DataRowView
    ...
    Sub addBarang()
        ...
        '    DRWPenjualanResep("jenis_model4") = "2"
        'Else
        '    DRWPenjualanResep("jenis_model4") = "1"
        'End If
    ...
    Sub addBarangKh()
        ...
        'End If
        'If cbInjeksi.Checked = True Then
        '    DRWPenjualanResepKh("jenis_model4") = "2"
        'Else
        '    DRWPenjualanResepKh("jenis_model4") = "1"
        'End If
    ...
    Sub tampilBarang()
        ...
        End If
        Try
            If pkdapo = "004" Or pkdapo = "005" Then
    ...
    Sub tampilBarangSemua()
        ...
        Try
            ...
            Else
                'DA = New OleDb.OleDbDataAdapter("select idddx_barang, kd_barang, LTRIM(RTRIM(nama_barang)) as nama_barang," & Stok & ", LTRIM(RTRIM(kd_satuan_kecil)) as kd_satuan_kecil, LTRIM(RTRIM(keterangan)) as keterangan from Barang_Farmasi WHERE stsaktif ='1' AND " & Stok & ">0 order by nama_barang", CONN)
                DA = New OleDb.OleDbDataAdapter("select bf.idx_barang, bf.kd_barang, LTRIM(RTRIM(bf.nama_barang)) as nama_barang,
                        " & Stok & ", LTRIM(RTRIM(bf.kd_satuan_kecil)) as kd_satuan_kecil, LTRIM(RTRIM(bf.keterangan)) as keterangan, 
                        max(ambl.tglexp) as tglexp 
                        from Barang_Farmasi as bf 
    ...
    Sub tampilPaket()
        ...
        If JenisRawat = 2 Then
            ...
            DA.Fill(DT)
            If DT.Rows.Count > 0 Then
                If IsDBNull(DT.Rows(0).Item("no_SJP")) Then
                    noKartu = "-"
                    noSep = "-"
                ElseIf DT.Rows(0).Item("no_SJP") = "" Then
                    noKartu = "-"
                    noSep = "-"
                Else
        ...
        Else
            ...
            If DT.Rows.Count > 0 Then
                ...
                ''batas
                'If IsDBNull(DT.Rows(0).Item("no_kartu")) And IsDBNull(DT.Rows(0).Item("no_SJP")) And IsDBNull(DT.Rows(0).Item("kd_icd_sku")) And IsDBNull(DT.Rows(0).Item("kd_icd_rm")) Then
                '	noKartu = "-"
                '	noSep = "-"
                '	kdICDSKU = "-"
                '	kdICDRM = "-"
                'Else
                '	If IsDBNull(DT.Rows(0).Item("kd_icd_sku")) Then
                '		kdICDSKU = "-"
                '		kdICDRM = DT.Rows(0).Item("kd_icd_rm")
                '	ElseIf IsDBNull(DT.Rows(0).Item("kd_icd_rm")) Then
                '		kdICDRM = "-"
    ...
    Private Sub btnSimpan_Click(sender As Object, e As EventArgs) Handles btnSimpan.Click
        ...
        cekPermintaanEMR()
        Dim noPermintaanObat As String
        If IsDBNull(DT.Rows(0).Item("no_permintaan_obat")) Then
            noPermintaanObat = ""
        Else
            noPermintaanObat = DT.Rows(0).Item("no_permintaan_obat")
        End If
        ...
        If MessageBox.Show("Data Penjualan sudah benar ...?", "", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
            ...
            Try
                ...
                ''''''''''''''''''''''''''''''''''TRANS KE APOTEK''''''''''''''''''''''''''''''''''''''''''''''''
                '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''' Simpan ap_jualr1
                sqlEditPenjualanObat = sqlEditPenjualanObat & vbCrLf &
                    "insert into ap_jualr1 (stsrawat, kdkasir, nmkasir, tanggal, notaresep, no_reg, 
                            no_rm, nama_pasien, kd_penjamin, nm_penjamin, kddokter, nmdokter, kdbagian, 
                            stsresep, totalpaket, totalpaket_bulat, totalnonpaket, totalnonpaket_bulat, 
                            totaldijamin, totaldijamin_bulat, totalselisih_bayar, totalselisih_bayar_bulat, 
                            kd_sub_unit, kd_sub_unit_asal, nama_sub_unit, jam, rsp_pulang, posting, 
                    ...
                    values ('" & StatusRawat & "', '" & Trim(FormLogin.LabelKode.Text) & "', 
                            ...
                            '" & Trim(kdDokter) & "', '" & Trim(NamaDokter) & "', '" & pkdapo & "', 'PKTUMUM', '" & Num_En_US(txtGrandTotal.DecimalValue) & "', 
                            '" & Num_En_US(txtGrandTotalBulat.DecimalValue) & "', '0', '0', '" & Num_En_US(txtGrandDijamin.DecimalValue) & "', 
                            '" & Num_En_US(txtGrandDijaminBulat.DecimalValue) & "', '" & Num_En_US(txtGrandIurBayar.DecimalValue) & "', 
                            '" & Num_En_US(txtGrandIurBayarBulat.DecimalValue) & "', '" & kdSubUnit & "', '" & kdSubUnit & "', 
    ...
    Private Sub btnSimpanKh_Click(sender As Object, e As EventArgs) Handles btnSimpanKh.Click
        ...
        If MessageBox.Show("Data Penjualan sudah benar ...?", "", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
            ...
            Try
                ...
                ''''''''''''''''''''''''''''''''''''''TRANS KE APOTEK'''''''''''''''''''''''''''''''''''''''''''
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''' 'Simpan ap_jualr1
                sqlEditPenjualanObatKh = sqlEditPenjualanObatKh & vbCrLf &
                    "insert into ap_jualr1 (stsrawat,kdkasir,nmkasir,tanggal,notaresep,no_reg,no_rm,
                        nama_pasien,kd_penjamin,nm_penjamin,kddokter,nmdokter,kdbagian,stsresep,totalpaket,
                        totalpaket_bulat,totalnonpaket,totalnonpaket_bulat,totaldijamin,totaldijamin_bulat,
                        totalselisih_bayar,totalselisih_bayar_bulat,kd_sub_unit,kd_sub_unit_asal,nama_sub_unit,jam,
                    ...
                    values ('" & StatusRawat & "', '" & Trim(FormLogin.LabelKode.Text) & "', '" & Trim(FormLogin.LabelNama.Text) & "', 
                        ...
                        'PKTKHUSUS', '" & Num_En_US(txtGrandTotalPaket.DecimalValue) & "', '" & Num_En_US(txtGrandTotalPaketBulat.DecimalValue) & "', 
                        '" & Num_En_US(txtGrandTotalNonPaket.DecimalValue) & "', '" & Num_En_US(txtGrandTotalNonPaketBulat.DecimalValue) & "', 
                        '" & Num_En_US(txtGrandTotalPaket.DecimalValue) & "', '" & Num_En_US(txtGrandTotalPaketBulat.DecimalValue) & "', 
                        '0', '0', '" & kdSubUnit & "', '" & kdSubUnit & "', '" & nmSubUnit & "', '" & Format(DTPJamAkhir.Value, "HH:mm:ss") & "', 
                        'N', '1', 'B',  '" & noPermintaanObat & "' )"
                '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''' 'Simpan ap_jualr2
                For i = 0 To gridDetailObatKh.RowCount - 2
    ...
    Sub cekPermintaanEMR()
        CMD = New OleDb.OleDbCommand("select no_permintaan_obat FROM ap_jualr1 WHERE tanggal='" & Format(DTPTanggalTrans.Value, "yyyy/MM/dd") & "' and notaresep='" & Trim(txtNoResep.Text) & "'", CONN)
        DA = New OleDb.OleDbDataAdapter(CMD)
        DT = New DataTable
        DA.Fill(DT)
    End Sub
    ...
    Private Sub btnHapusNotaKh_Click(sender As Object, e As EventArgs) Handles btnHapusNotaKh.Click
        If MessageBox.Show("Yakin transaksi ini akan dihapus?", "", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
            ...
            noPermintaanObat = noPermintaanObat
            ...
            Try
                ...
                '''' DELETE STATUS OBAT JADI TERLAYANI
                ...
                '''' DELETE STATUS OBAT RACIK TERLAYANI
    ...
    Sub cetakNotaBPJS2()
        ...
        Try
            ...
            rptLain.SetParameterValue("unit", nmSubUnit)
            rptLain.SetParameterValue("totalNonPaketBulat", txtGrandTotalNonPaketBulat.DecimalValue)
            rptLain.SetParameterValue("totalPaketBulat", txtGrandTotalPaketBulat.DecimalValue)
            rptLain.SetParameterValue("terbilang", bilang)
            FormCetak.CrystalReportViewer1.ReportSource = rptLain
            FormCetak.CrystalReportViewer1.Show()
            FormCetak.ShowDialog()
    ...
    Private Sub txtKodeObatKh_TextChanged(sender As Object, e As EventArgs) Handles txtKodeObatKh.TextChanged
    ...
    Private Sub txtJumlahObatEtiketModel3_TextChanged(sender As Object, e As EventArgs) Handles txtJumlahObatEtiketModel3.TextChanged
    ...
    Private Sub btnCetakDaftarEtiket_Click(sender As Object, e As EventArgs) Handles btnCetakDaftarEtiket.Click
        ...
        Dim rpt As New ReportDocument
        Try
            Dim str As String = Application.StartupPath & "\Report\etiketDaftar.rpt"
            rpt.Load(str)
            'FormCetak.CrystalReportViewer1.Refresh()
            rpt.SetDatabaseLogon(dbUser, dbPassword)
            rpt.SetParameterValue("tanggal", Format(DTPTanggalTrans.Value, "yyyy/MM/dd"))
