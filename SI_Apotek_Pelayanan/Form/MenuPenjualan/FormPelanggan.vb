...
Public Class FormPelanggan
    ...
    Private Sub gridPegawai_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles gridPegawai.CellContentClick
        If e.ColumnIndex = 0 Then
            If Not IsDBNull(gridPegawai.Rows(e.RowIndex).Cells("nama_pegawai").Value) Then
                ...
                txtUnit.Text = If(IsDBNull(gridPegawai.Rows(e.RowIndex).Cells("unit_kerja").Value), "-", If(gridPegawai.Rows(e.RowIndex).Cells("unit_kerja").Value = "", "-", gridPegawai.Rows(e.RowIndex).Cells("unit_kerja").Value))
    ...
    Private Sub gridPegawai_KeyPress(sender As Object, e As KeyPressEventArgs) Handles gridPegawai.KeyPress
        If e.KeyChar = Chr(13) Then
            ...
            If Not IsDBNull(gridPegawai.Rows(i).Cells(1).Value) Then
                ...
                txtUnit.Text = If(IsDBNull(gridPegawai.Rows(i).Cells("unit_kerja").Value), "-", If(gridPegawai.Rows(i).Cells("unit_kerja").Value = "", "-", gridPegawai.Rows(i).Cells("unit_kerja").Value))
    ...
    Sub NoPelanggan()
        Try
            ...
            CMD = New OleDbCommand("select max(kode_pelanggan) as kode_pelanggan from ap_pelanggan_apotik WHERE LEFT(kode_pelanggan,1)='" & kodePelanggan & "' AND CAST ((created_at) as DATE)='" & Format(DTPTanggalTrans.Value, "yyyy-MM-dd") & "' ORDER BY kode_pelanggan", CONN)
            DA = New OleDbDataAdapter(CMD)
            DT = New DataTable
            DA.Fill(DT)
            If IsDBNull(DT.Rows(0).Item("kode_pelanggan")) Then
                txtKodePelanggan.Text = kodePelanggan + Format(DTPTanggalTrans.Value, "yyMMdd") + "01"
            Else
                txtKodePelanggan.Text = Microsoft.VisualBasic.Right(DT.Rows(0).Item("kode_pelanggan").ToString, 2) + 1
    ...
    Private Sub FormPelanggan_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Dispose()
    End Sub
End Class