...
Public Class FormKoreksiTambah
    ...
    Sub NomorKoreksi()
        Try
            konek()
            CMD = New OleDb.OleDbCommand("SELECT max(nokoreksi) as nokoreksi FROM ap_koreksiapo_tambah WHERE YEAR(tanggal)='" & Year(DTPTanggalTrans.Value) & "'", CONN)
            DR = CMD.ExecuteReader
            DR.Read()
            If IsDBNull(DR.Item("nokoreksi")) Then
                txtNoKoreksi.Text = "KT" + Format(DTPTanggalTrans.Value, "yy") + "00001"
            Else
                txtNoKoreksi.Text = Microsoft.VisualBasic.Right(DR.Item("nokoreksi").ToString, 5) + 1
                If Len(txtNoKoreksi.Text) = 1 Then
                    txtNoKoreksi.Text = "KT" + Format(DTPTanggalTrans.Value, "yy") + "0000" & txtNoKoreksi.Text & ""
                ElseIf Len(txtNoKoreksi.Text) = 2 Then
                    txtNoKoreksi.Text = "KT" + Format(DTPTanggalTrans.Value, "yy") + "000" & txtNoKoreksi.Text & ""
                ElseIf Len(txtNoKoreksi.Text) = 3 Then
                    txtNoKoreksi.Text = "KT" + Format(DTPTanggalTrans.Value, "yy") + "00" & txtNoKoreksi.Text & ""
                ElseIf Len(txtNoKoreksi.Text) = 4 Then
                    txtNoKoreksi.Text = "KT" + Format(DTPTanggalTrans.Value, "yy") + "0" & txtNoKoreksi.Text & ""
                ElseIf Len(txtNoKoreksi.Text) = 5 Then
                    txtNoKoreksi.Text = "KT" + Format(DTPTanggalTrans.Value, "yy") + "" & txtNoKoreksi.Text & ""
                End If
    ...
    Private Sub FormKoreksiTambah_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Dispose()
    End Sub

    Private Sub FormKoreksiTambah_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub FormKoreksiTambah_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        setApo()
        Me.KeyPreview = True
        kosongkan()
        NomorKoreksi()
