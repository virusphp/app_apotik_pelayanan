...
Public Class FormReturPenjualan
    ...
    Dim BDDataPasienRJ, BDObatRJ, BDReturObatRJ As New BindingSource
    Dim DRWReturObatRJ As DataRowView
    Dim DSReturObatRJ As New DataSet
    Dim NamaPenjamin, kdPenjamin, kdDokter, NamaDokter, kdPoliklinik, noidBarang, Generik, kdJnsObat, KdKelObat, kdGolObat, kdPabrik, Formularium, Rekening, JenisObat, nmSubUnit, kdSubUnit, memStok, bilang As String
    ...
    Private Sub txtCariBarang_TextChanged(sender As Object, e As EventArgs) Handles txtCariBarang.TextChanged
        BDObatRJ.Filter = "nama_barang like '%" & txtCariBarang.Text & "%'"
    End Sub
    ...
    Sub addBarang()
        ...
        BDReturObatRJ.DataSource = DSReturObatRJ
        BDReturObatRJ.DataMember = "ReturObatRJ"

        BDReturObatRJ.AddNew()
        DRWReturObatRJ = BDReturObatRJ.Current
        DRWReturObatRJ("kdkasir") = Trim(FormLogin.LabelKode.Text)
        DRWReturObatRJ("nmkasir") = Trim(FormLogin.LabelNama.Text)
        DRWReturObatRJ("kdbagian") = pkdapo
        DRWReturObatRJ("tanggal") = DTPTanggalTrans.Value
        DRWReturObatRJ("notaretur") = Trim(txtNoRetur.Text)
        DRWReturObatRJ("no_reg") = Trim(txtNoReg.Text)
        DRWReturObatRJ("no_rm") = Trim(txtRM.Text)
        DRWReturObatRJ("nmpasien") = Trim(txtNamaPasien.Text)
        DRWReturObatRJ("umurthn") = Trim(txtUmurThn.Text)
        DRWReturObatRJ("umurbln") = Trim(txtUmurBln.Text)
        DRWReturObatRJ("kd_penjamin") = Trim(kdPenjamin)
        DRWReturObatRJ("nm_penjamin") = Trim(NamaPenjamin)
        DRWReturObatRJ("urut") = 1
        DRWReturObatRJ("noid") = Trim(noidBarang)
        DRWReturObatRJ("kd_barang") = Trim(txtKodeObat.Text)
        DRWReturObatRJ("idx_barang") = Trim(txtIdxBarang.Text)
        DRWReturObatRJ("nama_barang") = Trim(lblNamaObat.Text)
        DRWReturObatRJ("generik") = Trim(Generik)
        DRWReturObatRJ("kd_jns_obat") = Trim(kdJnsObat)
        DRWReturObatRJ("kd_gol_obat") = Trim(kdGolObat)
        DRWReturObatRJ("kd_kel_obat") = Trim(KdKelObat)
        DRWReturObatRJ("kdpabrik") = Trim(kdPabrik)
        DRWReturObatRJ("rek_p") = Trim(Rekening)
        DRWReturObatRJ("formularium") = Trim(Formularium)
        DRWReturObatRJ("tglresep") = DTPTanggalResep.Value
        DRWReturObatRJ("notaresep") = Trim(txtNotaResep.Text)
        DRWReturObatRJ("kddokter") = Trim(kdDokter)
        DRWReturObatRJ("nmdokter") = Trim(NamaDokter)
        DRWReturObatRJ("hrgppn") = txtHarga.DecimalValue
        DRWReturObatRJ("jmlretpkt") = txtRetPaket.DecimalValue
        DRWReturObatRJ("jmlretnpkt") = txtRetNonPaket.DecimalValue
        DRWReturObatRJ("totalqty") = txtJumlahRetur.DecimalValue
        DRWReturObatRJ("nmsatuan") = Trim(txtSatuan.Text)
        DRWReturObatRJ("jmlhrgpkt") = txtJmlHargaPaket.DecimalValue
        DRWReturObatRJ("jmlhrgnpkt") = txtJmlHargaNonPaket.DecimalValue
        DRWReturObatRJ("jmlhrgret") = txtTotalHargaRetur.DecimalValue
        DRWReturObatRJ("dijamin") = txtDijamin.DecimalValue
        DRWReturObatRJ("iurpasien") = txtIurPasien.DecimalValue
        DRWReturObatRJ("jns_obat") = Trim(JenisObat)

        BDReturObatRJ.EndEdit()

        gridDetailObat.DataSource = Nothing
        gridDetailObat.DataSource = BDReturObatRJ
    ...
    Sub tampilObat()
        Try
            ...
            DS = New DataSet
            DA.Fill(DS, "ObatRJ")
            BDObatRJ.DataSource = DS
            BDObatRJ.DataMember = "ObatRJ"
            With gridBarang
                .DataSource = Nothing
                .DataSource = BDObatRJ
    ...
    Sub detailPasien()
        ...
        CMD = New OleDbCommand("select Sub_Unit.nama_sub_unit, Sub_Unit.kd_sub_unit from sub_unit 
                            where kd_sub_unit='" & kdPoliklinik & "'", CONN)
        DA = New OleDbDataAdapter(CMD)
        DT = New DataTable
        DA.Fill(DT)
        If DT.Rows.Count > 0 Then
    ...
    Sub TampilPasieRJ()
        Try
            DA = New OleDbDataAdapter("SELECT 
                    ...
                    WHERE reg.jns_rawat='1' AND reg.status_keluar<>1 AND reg.tgl_reg = '" & Format(DTPTanggalTrans.Value, "yyyy/MM/dd") & "'
    ...
    Sub KosongkanHeader()
        TglServer()
        DSReturObatRJ = Table.BuatTabelReturObatRJ("ReturObatRJ")
