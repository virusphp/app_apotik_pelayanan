...
Public Class FormRincianObatPasienRI
    ...
    Private Sub btnView_Click(sender As Object, e As EventArgs) Handles btnView.Click
        Try
            CMD = New OleDb.OleDbCommand("select SUM(totalharga) from ap_jualr2 where no_rm='" & txtRM.Text & "' and (tanggal BETWEEN '" & Format(DTPAwal.Value, "yyyy-MM-dd") & "' AND '" & Format(DTPAkhir.Value, "yyyy-MM-dd") & "')", CONN)
            DA = New OleDb.OleDbDataAdapter(CMD)
            DT = New DataTable
            DA.Fill(DT)
            totalHarga = DT.Rows(0).Item(0)
