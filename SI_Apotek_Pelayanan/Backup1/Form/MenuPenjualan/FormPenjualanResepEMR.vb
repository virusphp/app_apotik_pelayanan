...
Public Class FormPenjualanResepEMR
        ...
        Try
            ...
            ElseIf cmbJenisRawat.SelectedIndex = 2 Then
                CMD = New OleDb.OleDbCommand("select registrasi.tgl_reg as tgl_reg,registrasi.no_reg as no_reg, 
                        ...
                        AND registrasi.status_keluar <> '2' 
                        AND registrasi.no_reg='" & gridPasien.Rows(e.RowIndex).Cells(2).Value & "' 
                        AND registrasi.no_reg IN (Select no_reg from kwitansi_header where no_reg='" & gridPasien.Rows(e.RowIndex).Cells(2).Value & "') order by registrasi.no_reg", CONN)
                DA = New OleDb.OleDbDataAdapter(CMD)
                DT = New DataTable
                DA.Fill(DT)
                If DT.Rows.Count > 0 Then
    ...
    Private Sub gridPasien_KeyPress(sender As Object, e As KeyPressEventArgs) Handles gridPasien.KeyPress
        ...
        If e.KeyChar = Chr(13) Then
            If cmbJenisRawat.SelectedIndex = 0 Then
                CMD = New OleDb.OleDbCommand("SELECT Registrasi.tgl_reg as tgl_reg, Registrasi.no_reg as no_reg, Registrasi.no_RM, Pasien.nama_pasien, Sub_Unit.nama_sub_unit, Registrasi.jns_rawat, Registrasi.jenis_pasien, Registrasi.status_keluar FROM Registrasi INNER JOIN Pasien ON Registrasi.no_RM = Pasien.no_RM INNER JOIN Rawat_Jalan ON Registrasi.no_reg = Rawat_Jalan.no_reg INNER JOIN Sub_Unit ON Rawat_Jalan.kd_poliklinik = Sub_Unit.kd_sub_unit where registrasi.tgl_reg='" & Format(DTPPasienReg.Value, "yyyy/MM/dd") & "'  AND registrasi.jns_rawat='" & JenisRawat & "' AND registrasi.status_keluar <> '2' AND registrasi.no_reg='" & gridPasien.Rows(i).Cells(2).Value & "' AND registrasi.no_reg IN (Select no_reg from kwitansi_header where no_reg='" & gridPasien.Rows(i).Cells(2).Value & "' AND jenis_pasien<>'Umum') order by registrasi.no_reg", CONN)
                DA = New OleDb.OleDbDataAdapter(CMD)
                DT = New DataTable
                DA.Fill(DT)
                If DT.Rows.Count > 0 Then
