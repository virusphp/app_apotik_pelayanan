...
Public Class FormLaporanRekapHarianPenjualanResepNonPaket
    ...
    Sub JumlahItem()
        ...
        Next
        txtTotalItem.DecimalValue = HitungTotal
    End Sub
    ...
    Sub TampilGrid1()
        ...
        Try
            ...
            'konek()
            DA = New OleDb.OleDbDataAdapter("select tanggal,notaresep,racik,jmlhari from ap_jualr2 where kdbagian" + XopBagian + "'" & kdBagian & "' and stsrawat" + XopStatus + "'" & JenisPasien & "' and tanggal >= '" & Format(DTPTanggalAwal.Value, "yyyy/MM/dd") & "' AND tanggal <= '" & Format(DTPTanggalAkhir.Value, "yyyy/MM/dd") & "' AND kd_penjamin " + XopPenjamin + "'" & kdPenjamin & "' AND totalnonpaket > 0", CONN)
            DSRacik = New DataSet
            ...
            ''''''''''''''data stok totalobat
            'konek()
            DA = New OleDb.OleDbDataAdapter("select tanggal,notaresep,kd_jns_obat,totalnonpaket from ap_jualr2 where kdbagian" + XopBagian + "'" & kdBagian & "' and stsrawat" + XopStatus + "'" & JenisPasien & "' and tanggal >= '" & Format(DTPTanggalAwal.Value, "yyyy/MM/dd") & "' AND tanggal <= '" & Format(DTPTanggalAkhir.Value, "yyyy/MM/dd") & "' AND kd_penjamin " + XopPenjamin + "'" & kdPenjamin & "' AND kd_jns_obat=1 AND totalnonpaket > 0", CONN)
            ...
            If BDLaporanHarianPenjualanResep.Count > 0 Then
                ...
                For i = 1 To BDLaporanHarianPenjualanResep.Count
                    ...
                    DRWLaporanHarianPenjualanResep("totalobt") = DSTotalObat.Tables("TotalObat").Compute("sum(totalnonpaket)", "notaresep = '" & Trim(DRWLaporanHarianPenjualanResep.Item("notaresep").ToString) & "'")
                    If IsDBNull(DRWLaporanHarianPenjualanResep("totalobt")) Then
                        DRWLaporanHarianPenjualanResep("totalobt") = 0
                    Else
                        Dim a As Decimal
                        a = DSTotalObat.Tables("TotalObat").Compute("sum(totalnonpaket)", "notaresep = '" & Trim(DRWLaporanHarianPenjualanResep.Item("notaresep").ToString) & "'")
                        a = a.ToString("0.00")
    ...
    Private Sub FormLaporanRekapHarianPenjualanResepNonPaket_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        kosongkan()
        ListBagian()
        ListPenjamin()
        ListJenisPasien()
    ...
    Private Sub btnExcelTab1_Click(sender As Object, e As EventArgs) Handles btnExcelTab1.Click
        If MessageBox.Show("Apakah data akan di eksport ke excel?", "", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
            Try
                ...
                Dim workbook As IWorkbook = excelEngine.Excel.Workbooks.Open(Application.StartupPath & "\Report\LaporanRekapHarianPenjualanResepNonPaketXLSIO.xlsx")
                ...
                workbook.SaveAs("Laporan Rekap Harian Penjualan Resep Non Paket.xlsx")
                workbook.Close()
                excelEngine.Dispose()
                System.Diagnostics.Process.Start("Laporan Rekap Harian Penjualan Resep Non Paket.xlsx")
            Catch ex As Exception
                MsgBox(ex.Message)
            End Try
        End If
