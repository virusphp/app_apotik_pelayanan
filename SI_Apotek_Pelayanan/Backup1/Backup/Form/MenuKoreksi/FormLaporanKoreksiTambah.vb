...
Public Class FormLaporanKoreksiTambah
    ...
    Sub tampilPerTanggal()
        Try
            konek()
            DA = New OleDb.OleDbDataAdapter("select nmkasir, nokoreksi, tanggal, idx_barang, kd_barang, RTRIM(LTRIM(nama_barang)) as nama_barang, jml, RTRIM(LTRIM(nmsatuan)) as nmsatuan, harga, jmlharga, RTRIM(LTRIM(keterangan)) as keterangan from ap_koreksiapo_tambah where tanggal >= '" & Format(DTPTanggalAwalTab1.Value, "yyyy/MM/dd") & "' and tanggal <= '" & Format(DTPTanggalAkhirTab1.Value, "yyyy/MM/dd") & "' and kdbagian='" & pkdapo & "' order by tanggal,noid", CONN)
            DS = New DataSet
            DA.Fill(DS, "KoreksiTambahPerTanggal")
            BDPertanggal.DataSource = DS
            BDPertanggal.DataMember = "KoreksiTambahPerTanggal"
            With GridTab1
                .DataSource = Nothing
    ...
    Sub tampilPerBarang()
        Try
            ...
            DS = New DataSet
            DA.Fill(DS, "KoreksiTambahPerBarang")
            BDPerBarang.DataSource = DS
            BDPerBarang.DataMember = "KoreksiTambahPerBarang"
            With GridTab2
                .DataSource = Nothing
                .DataSource = BDPerBarang
                .Columns(0).HeaderText = "Kd Kasir"
                .Columns(1).HeaderText = "No Koreksi"
                .Columns(2).HeaderText = "Tanggal"
                .Columns(3).HeaderText = "ID Barang"
                .Columns(4).HeaderText = "Kode Barang"
    ...
    Private Sub FormLaporanKoreksiTambah_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Dispose()
    End Sub

    Private Sub FormLaporanKoreksiTambah_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        setApo()
        kosongkan1()
        kosongkan2()
    End Sub
    ...
    Private Sub btnExcelTab1_Click(sender As Object, e As EventArgs) Handles btnExcelTab1.Click
        If MessageBox.Show("Apakah data akan di eksport ke excel?", "", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
            Try
                Dim dtXls As DataTable = CType(DS.Tables("KoreksiTambahPerTanggal"), DataTable)
                Dim excelEngine As New ExcelEngine
                excelEngine.Excel.DefaultVersion = ExcelVersion.Excel2007
                Dim workbook As IWorkbook = excelEngine.Excel.Workbooks.Open(Application.StartupPath & "\Report\LaporanKoreksiTambahPerTanggalXLSIO.xlsx")
                ...
                workbook.SaveAs("Laporan Koreksi Tambah Per Tanggal.xlsx")
                workbook.Close()
                excelEngine.Dispose()
                System.Diagnostics.Process.Start("Laporan Koreksi Tambah Per Tanggal.xlsx")
            Catch ex As Exception
                MsgBox(ex.Message)
            End Try
        End If
    ...
    Private Sub btnExcelTab2_Click(sender As Object, e As EventArgs) Handles btnExcelTab2.Click
        If MessageBox.Show("Apakah data akan di eksport ke excel?", "", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
            Try
                Dim dtXls As DataTable = CType(DS.Tables("KoreksiTambahPerBarang"), DataTable)
                Dim excelEngine As New ExcelEngine
                excelEngine.Excel.DefaultVersion = ExcelVersion.Excel2007
                Dim workbook As IWorkbook = excelEngine.Excel.Workbooks.Open(Application.StartupPath & "\Report\LaporanKoreksiTambahPerBarangXLSIO.xlsx")
                Dim sheet As IWorksheet = workbook.Worksheets(0)
                sheet.Range("B7").Text = DTPTanggalAwalTab1.Text
                sheet.Range("B8").Text = DTPTanggalAkhirTab1.Text
                sheet.Range("B9").Text = pnmapo
                ...
                marker.ApplyMarkers()
                workbook.Version = ExcelVersion.Excel2007
                workbook.SaveAs("Laporan Koreksi Tambah Per Barang.xlsx")
                workbook.Close()
                excelEngine.Dispose()
                System.Diagnostics.Process.Start("Laporan Koreksi Tambah Per Barang.xlsx")
            Catch ex As Exception
                MsgBox(ex.Message)
            End Try
        End If
